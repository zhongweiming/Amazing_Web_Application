...
checkAdminLogined();
//$uploadPath = "Project/Trade/uploads";
$order = $_REQUEST['order'] ? $_REQUEST['order'] : null;
$orderBy = $order ? "order by p." . $order : null;
$keywords = $_REQUEST['keywords'] ? $_REQUEST['keywords'] : null;
$where = $keywords ? "where p.pName like '%{$keywords}%'" : null;
//得到数据库中所有商品
$sql = "select p.id,p.pName,p.pStyleID,p.pNum,p.marketPrice,p.internetPrice,p.pDesc,p.publishTime,p.isShow,p.isHot,c.cName from product as p join cate c on p.cId=c.id {$where}  ";
...
$sql = "select p.id,p.pName,p.pStyleID,p.pNum,p.marketPrice,p.internetPrice,p.pDesc,p.publishTime,p.isShow,p.isHot,c.cName from product as p join cate c on p.cId=c.id {$where} {$orderBy} limit {$offset},{$pageSize}";
$rows = fetchAll($sql);
            ...
            <div class="text">
                ...
                <div class="bui_select">
                    <select id="" class="select" onchange="change(this.value)">
                        <option>-请选择-</option>
                        <option value="internetPrice asc">由低到高</option>
                        <option value="internetPrice desc">由高到底</option>
                    </select>
                </div>
            </div>
            <div class="text">
                ...
                <div class="bui_select">
                    <select id="" class="select" onchange="change(this.value)">
                        <option>-请选择-</option>
                        <option value="publishTime desc">最新发布</option>
                        <option value="publishTime asc">历史发布</option>
                    </select>
                </div>
            </div>
            <div class="text">
    ...
    <table class="table" cellspacing="0" cellpadding="0">
        ...
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td><?php echo $row['internetPrice']; ?>元</td>
                <td align="center">
                    ...
                    <div id="showDetail<?php echo $row['id']; ?>" style="display:none;">
                        <table class="table" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                <td width="20%" align="right">二手价格</td>
                                <td><?php echo $row['internetPrice']; ?></td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    foreach ($proImgs as $img):
                                        ...
                                        <img width="100" height="100"
                                             src="../uploads/<?php echo $img['albumPath']; ?>"
                                             alt=""/> &nbsp;&nbsp;
                                    <?php endforeach; ?>
                                </td>
                            </tr>
